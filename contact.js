(() => {
  ...
  const copy = {
    en: {
      ...
      performance:{kicker:'Measured engine benchmark',title:'16.643× measured realtime simulation on an 8 GB laptop.',lead:'A 10-hour simulation with 600 Vehicles completed in 2,163.085 seconds of actual wall-clock time — 36 minutes 3.085 seconds.',metrics:[['16.643×','Realtime factor'],['600','Vehicles'],['10 h','Simulated time'],['36m 3.085s','Wall-clock time'],['88,941','Completed jobs'],['8,894.1 / h','Completed moves / simulated hour']],machineTitle:'Benchmark environment',machine:[['CPU','11th Gen Intel Core i5-1130G7'],['RAM','8.00 GB (7.70 GB usable)'],['System','64-bit Windows · x64'],['Run mode','Mixed · Animation ON + Ultimate Speed']],note:'Measured on Aug 22, 2026. Mixed mode means the run used both Animation ON and Ultimate Speed. This is a measured end-to-end result, not a claimed maximum. Performance varies with layout complexity, traffic density, simulation settings, and hardware.'},
    ...
    ko: {
      ...
      performance:{kicker:'Simulation Engine 실측 성능',title:'8GB 노트북에서 16.643배속 실측.',lead:'600대 Vehicle로 10시간 시뮬레이션을 실제 2,163.085초, 즉 36분 3.085초에 완료했습니다.',metrics:[['16.643×','실측 시뮬레이션 배속'],['600','Vehicle'],['10시간','시뮬레이션 시간'],['36분 3.085초','실제 실행시간'],['88,941','완료 반송'],['8,894.1 / h','시뮬레이션 시간당 완료 반송']],machineTitle:'측정 환경',machine:[['CPU','11th Gen Intel Core i5-1130G7'],['RAM','8.00 GB (7.70 GB 사용 가능)'],['시스템','64비트 Windows · x64'],['실행 모드','Mixed · Animation ON + Ultimate Speed']],note:'2026-08-22 실측 결과입니다. Mixed mode는 한 Run에서 Animation ON과 Ultimate Speed를 모두 사용했다는 의미입니다. 따라서 16.643배속은 특정 조건에서 측정한 전체 Run 실측값이며 최대 성능을 보장하는 수치가 아닙니다. 도면 복잡도, 교통 밀도, 시뮬레이션 설정과 하드웨어에 따라 성능은 달라질 수 있습니다.'},
    ...
    'zh-CN': {
      ...
      performance:{kicker:'Simulation Engine 实测性能',title:'8 GB 笔记本实测 16.643× 实时倍率。',lead:'600 辆 Vehicle 的 10 小时仿真在 2,163.085 秒实际时间内完成，即 36 分 3.085 秒。',metrics:[['16.643×','实测实时倍率'],['600','Vehicles'],['10 小时','仿真时间'],['36 分 3.085 秒','实际运行时间'],['88,941','完成任务'],['8,894.1 / h','每仿真小时完成搬送']],machineTitle:'测试环境',machine:[['CPU','11th Gen Intel Core i5-1130G7'],['RAM','8.00 GB（7.70 GB 可用）'],['系统','64 位 Windows · x64'],['运行模式','Mixed · Animation ON + Ultimate Speed']],note:'测试日期：2026-08-22。Mixed mode 表示同一次 Run 中同时使用了 Animation ON 和 Ultimate Speed。因此 16.643× 是该条件下的端到端实测结果，并非宣称的最大性能。实际性能会随布局复杂度、交通密度、仿真设置和硬件变化。'},
    ...
    es: {
      ...
      performance:{kicker:'Rendimiento medido del Simulation Engine',title:'16,643× de factor de tiempo real medido en un portátil con 8 GB.',lead:'Una simulación de 10 horas con 600 Vehicles terminó en 2.163,085 segundos de tiempo real, es decir, 36 min 3,085 s.',metrics:[['16,643×','Factor de tiempo real'],['600','Vehicles'],['10 h','Tiempo simulado'],['36m 3,085s','Tiempo real de ejecución'],['88.941','Trabajos completados'],['8.894,1 / h','Movimientos completados / hora simulada']],machineTitle:'Entorno de medición',machine:[['CPU','11th Gen Intel Core i5-1130G7'],['RAM','8,00 GB (7,70 GB utilizables)'],['Sistema','Windows de 64 bits · x64'],['Modo','Mixed · Animation ON + Ultimate Speed']],note:'Medido el 22 de agosto de 2026. Mixed mode significa que la misma ejecución utilizó Animation ON y Ultimate Speed. Por tanto, 16,643× es un resultado integral medido bajo estas condiciones, no una afirmación de rendimiento máximo. El rendimiento varía según la complejidad del layout, la densidad de tráfico, la configuración y el hardware.'},
    ...
    ja: {
      ...
      performance:{kicker:'Simulation Engine 実測性能',title:'8 GBノートPCで16.643×の実測リアルタイム倍率。',lead:'600台のVehicleによる10時間シミュレーションを、実時間2,163.085秒、つまり36分3.085秒で完了しました。',metrics:[['16.643×','実測リアルタイム倍率'],['600','Vehicles'],['10時間','シミュレーション時間'],['36分3.085秒','実行実時間'],['88,941','完了Job'],['8,894.1 / h','シミュレーション1時間あたり完了搬送']],machineTitle:'測定環境',machine:[['CPU','11th Gen Intel Core i5-1130G7'],['RAM','8.00 GB（使用可能 7.70 GB）'],['システム','64-bit Windows · x64'],['実行モード','Mixed · Animation ON + Ultimate Speed']],note:'2026-08-22の実測結果です。Mixed modeは1回のRunでAnimation ONとUltimate Speedの両方を使用したことを意味します。16.643×はこの条件でのエンドツーエンド実測値であり、最大性能を保証する数値ではありません。レイアウトの複雑度、交通密度、設定、ハードウェアにより性能は変動します。'},
