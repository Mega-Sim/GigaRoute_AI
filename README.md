...
- 상단 헤더의 Windows 버튼은 Issue #37부터 `Mega-Sim/GigaRoute_AI_Simulation_Demo`의 전체 Releases 페이지로 연결하고, Linux 버튼은 Issue #35부터 비활성화합니다. Auto Simulation 제품 카드와 하단 CTA의 플랫폼별 다운로드 동선은 기존대로 유지합니다. 모든 언어 페이지 우측 하단에는 스크롤 후 나타나는 `TOP` 버튼을 제공해 최상단으로 부드럽게 이동합니다.
